...
In the [LWAI Prediction index stage](/fusion/{latest-fusion}/bzik28/lwai-prediction) and the [LWAI Prediction query stage](/fusion/{latest-fusion}/hnuyky/lwai-prediction), the `useCaseConfig entityTypeMap` parameter only supports a string. Therefore, the string entered in Fusion is converted to a JSON string, which is required in the Lucidworks AI `entityTypeMap` variable. For example:
