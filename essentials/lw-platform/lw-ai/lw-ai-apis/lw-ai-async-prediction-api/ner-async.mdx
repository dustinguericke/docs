...
|"useCaseConfig" |`"entityTypeMap": {"ENTITY": ["exampleA", "exampleB"], "ENTITY1": ["exampleC", "exampleD"]}`
...
In the [LWAI Prediction index stage](/lw-platform/ai/bzik28/lucidworks-ai-prediction-index-stage) and the [LWAI Prediction query stage](/lw-platform/ai/hnuyky/lucidworks-ai-prediction-query-stage), the `useCaseConfig entityTypeMap` parameter only supports a string. Therefore, the string entered in Fusion is converted to a JSON string, which is required in the Lucidworks AI `entityTypeMap` variable. For example:
