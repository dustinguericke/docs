...
For information about model training, biases, and safety features for those models, refer to the documentation provided by the model creators. Hugging Face is one information source, and links to information about Lucidworks-hosted models are provided in [Available Lucidworks-hosted models](#available-lucidworks-hosted-models).
...
Each request requires an `apiKey`, `googleProjectId`, and `googleRegion`. There are no defaults for any of these fields. The value for `apiKey` is the Google Vertex AI service account key. For more information, see [Create a Google service account key](https://cloud.google.com/iam/docs/keys-create-delete#creating).

#### Generate service account key

Create a Google Vertex AI service account key with the [Vertex AI user](https://cloud.google.com/iam/docs/understanding-roles#aiplatform.user) role enabled, and then base64 encode the key:
...
# 2. Assign the Vertex AI User role to the service account
...
base64 $KEY_FILE > ${KEY_FILE}.b64

# Optional: print base64 string to terminal
cat ${KEY_FILE}.b64
```

#### Available Google Vertex AI models

The supported Google Vertex AI models are `gemini-2.0-flash` and `gemini-2.0-flash-lite`.
